...
typedef struct{ //estrutura que define uma pessoa com nome e cpf
	char* nome;
	int cpf;
} PESSOA;


typedef struct{ //estrutura que define um cliente
	PESSOA pessoa; //todo cliente é uma pessoa única
	char tipo; //se o cliente é especial ou comum
	int valorContaCorrente; //saldo na conta corrente
	int valorDaDivida; //dívida decorrente dos emprétimos desse cliente
} CLIENTE;


typedef struct{ //estrutura que define um gerente
	PESSOA pessoa; //todo gerente é uma pessoa única
	CLIENTE* clientes[20]; //todo gerente pode ter no máximo 20 clientes, essa é alista com os ponteiro de cada cliente do gerente
	int numClientes; //quantidade de clientes do gerente, que são no máximo 20
} GERENTE;


typedef struct{ //estrutura que deifne um banco
	GERENTE* gerentes[10]; //lista de ponteiros para os gerentes que trabalham nesse banco
	int numGerentes; //no máximo 10 gerentes por banco
} BANCO;
...
/* Funcao para a criacao de um novo cliente comum.
...
/* Funcao para a criacao de um novo cliente especial.
   ...
   Função cria um cliente comum e então faz ele um cliente especial
*/
 ...
 *   Caso o valor do parametro seja menor ou igual a zero, a funcao deve retornar false (result1)
 ...
 *         dividaMaximaEspecial (para clientes especiais), a funcao deve retornar false (result2.1 e result2.2)
 *     2a) caso contrario, o campo valorDaDivida deve ser incrementado em valor, o campo
 *         valorContaCorrente deve ser incrementado em valor e a funcao deve retornar true (result3)
 */
bool obterEmprestimo(CLIENTE* c1, int valor) {

	if(valor<=0) return false; //(result1)

	int dividaCliente = (c1->valorDaDivida)+valor;
	if(c1->tipo == 'C' && dividaCliente > dividaMaxima) {
		return false; //(result2.1)
	} else if(c1->tipo == 'E' && dividaCliente > dividaMaximaEspecial) {
		return false; //(result2.2)
	} else {
 ...
 *         valorContaCorrente, a funcao deve retornar false (result 2.1 e result2.2)
 *     2a) caso contrario, o campo valorDaDivida deve ser decrementado em valor, o campo
 *         valorContaCorrente deve ser decrementado em valor e a funcao deve retornar true (result3)
 */
bool pagarEmprestimo(CLIENTE* c1, int valor) {

	if(valor<=0) return false; //(result1)
	if(valor>c1->valorDaDivida || valor>c1->valorContaCorrente) {
		return false; //(result2.1 e result2.2)
	} else {
		c1->valorDaDivida -= valor;
		c1->valorContaCorrente -= valor;
		return true; //(result3)
 ...
 * Caso o numero de gerentes seja igual a 10, nao deve adicionar e deve retornar false (result1).
 * Caso contrario, ha duas situacoes: 
 *   1a: o gerente ja consta no arranjo de gerentes (verifique isso usando o numero do cpf),
 *       neste caso o gerente nao deve ser reinserido e a funcao deve retornar false; (result2)
 ...
 *       incrementado em 1 e a funcao deve retornar true. (result3)
 */
bool adicionarGerente(BANCO* b1, GERENTE* gerente){
	if (b1->numGerentes == 10) return false; //result1
	int x;
	for (x=0;x<b1->numGerentes;x++){
		if (b1->gerentes[x]->pessoa.cpf == gerente->pessoa.cpf) return false; //result2
	}
